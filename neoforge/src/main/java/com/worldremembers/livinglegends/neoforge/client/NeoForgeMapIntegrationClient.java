...
final class NeoForgeMapIntegrationClient {
    ...
    private NeoForgeMapIntegrationClient() {
    }

    static void registerBuiltInIntegrations(Logger logger) {
        NeoForgeMapIntegrationClient.logger = logger;
    ...
    private static synchronized void refreshBuiltInIntegrations() {
        if (XAERO_MINIMAP_INSTALLED && !INTEGRATIONS.containsKey(XAERO_PROVIDER_ID)) {
            try {
                installIntegration(new XaeroNeoForgeIntegration(logger));
        ...
        if (FTB_CHUNKS_INSTALLED && !INTEGRATIONS.containsKey(FTB_CHUNKS_PROVIDER_ID)) {
            try {
                installIntegration(new FtbChunksNeoForgeIntegration(logger));
            } catch (RuntimeException | LinkageError exception) {
                if (logger != null) {
    ...
    private static void refreshCurrentJournal() {
        ...
        if (client.screen instanceof WorldJournalScreen journal) {
    ...
    static void tick(Minecraft client) {
        ...
        if (client.player == null || client.level == null || DESTINATIONS.isEmpty()) {
            return;
        }
    ...
    private static void notifyDestinationFailure(DestinationUpdateResult result) {
        ...
        if (client != null && client.gui != null) {
            client.gui.getChat().addMessage(Component.translatable("living_legends.journal.error.destination_failed"));
        }
    }
    ...
    private static MapPlaceDescriptor resolveClientText(MapPlaceDescriptor place) {
        ...
        String displayName = place.manualName()
                ...
                : WorldRemembersLivingLegendsNeoForgeClientNameResolver.resolve(place.nameRecipe()).getString();
        if (displayName == null
                || displayName.isBlank()
    ...
    private static String buildTooltip(MapPlaceDescriptor place, String displayName) {
        ...
        String placeType = Component.translatable(placeTypeKey(place.placeType())).getString();
    ...
    private static PlaceMapIntegration compositeIntegration() {
        ...
        return new NeoForgeCompositePlaceMapIntegration(INTEGRATIONS.values());
    }
}
