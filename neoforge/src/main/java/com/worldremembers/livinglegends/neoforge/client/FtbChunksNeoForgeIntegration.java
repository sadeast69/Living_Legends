...
final class FtbChunksNeoForgeIntegration implements PlaceMapIntegration {
    ...
    FtbChunksNeoForgeIntegration(Logger logger) {
        this.logger = logger;
    ...
    private static boolean detectFtbChunks() {
        try {
            return ModList.get().isLoaded(PROVIDER_ID);
        } catch (RuntimeException | LinkageError exception) {
            return false;
        }
    }
    ...
    private Optional<WaypointManager> currentWaypointManager() {
        ...
        return waypointManager(client.level.dimension());
    }
    ...
    private static Optional<WaypointManager> waypointManager(ResourceKey<Level> dimension) {
        try {
    ...
    private static ResourceKey<Level> dimension(String dimensionId) {
        ResourceLocation id = ResourceLocation.tryParse(dimensionId(dimensionId));
        if (id == null) {
    ...
    private record TrackedWaypoint(String dimensionId, BlockPos pos, String name) {
        private TrackedWaypoint {
            dimensionId = FtbChunksNeoForgeIntegration.dimensionId(dimensionId);
