...
final class WorldRemembersLivingLegendsFabricNotifications {
    ...
    private WorldRemembersLivingLegendsFabricNotifications() {
    }

    static void notifyPlaceCreated(Object serverObject, NamedPlace place, Logger logger) {
        ...
        List<ServerPlayerEntity> recipients = recipients(server, place, settings);
        ...
        MutableText message = Text.translatable(messageKey, displayName(place));
        ...
        if (settings.includeDimension) {
            message.append(Text.literal(" "))
                    .append(Text.translatable("living_legends.message.place_created.dimension", place.dimensionId()));
        }
        if (settings.includePlaceType) {
            message.append(Text.literal(" "))
                    .append(Text.translatable("living_legends.message.place_created.type", place.placeType().name()));
        }
    ...
    private static MutableText displayName(NamedPlace place) {
        if (place.manuallyRenamed() && !place.manualName().isBlank()) {
            return Text.literal(place.manualName());
        }
        return WorldRemembersLivingLegendsFabricNameResolver.resolve(place.nameRecipe());
    }

    private static List<ServerPlayerEntity> recipients(
        ...
        for (ServerPlayerEntity player : players) {
            ...
            long dx = player.getBlockPos().getX() - place.center().x();
            long dy = player.getBlockPos().getY() - place.center().y();
            long dz = player.getBlockPos().getZ() - place.center().z();
